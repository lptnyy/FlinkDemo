...
public class CatalogContentSvAggregate implements AggregateFunction<DataLog, Object, Object> {
...
//        RedisUtil redisUtil = new RedisUtil();
        ...
        if (concurrentHashMap.get(key) != null) {
            ...
            catalogContent.setShares(catalogContent.getShareVisitors() +1);
        } else {
            catalogContent.setCatalogId(dataLog.getTid());
            catalogContent.setShareVisitors(1);
        ...
        if (!RedisUtil.hasKey(str)) {
            catalogContent.setShareVisitorNumber(1);
            RedisUtil.set(str, dataLog.getUid());
            RedisUtil.expire(str, 86400L);
        } else {
            catalogContent.setShareVisitorNumber(0);
        }
