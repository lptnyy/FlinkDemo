 ...
 */
@Slf4j
public class MysqlDataFlowDaySkin extends RichSinkFunction<ConcurrentHashMap<String, PerformanceDay>> {
        ...
        value.forEach((k, v) -> {
            try {
                ...
                if (performanceDay == null) {
                    v.setContributions(v.getVisitors() * 2 + v.getBrowses() + v.getTransponds() * 3);
                    this.performanceDayMapper.insert(v);
                } else {
                    performanceDay.setShares(performanceDay.getShares() + v.getShares());
                    performanceDay.setVisitors(performanceDay.getVisitors() + v.getVisitors());
                    ...
                    performanceDay.setContributions(performanceDay.getVisitors() * 2
                            + performanceDay.getBrowses() + performanceDay.getTransponds() * 3);
